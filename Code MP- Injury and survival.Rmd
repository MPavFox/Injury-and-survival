...
#ALL INJURIES##
#Simplest model
logit_fem1 = glmer(event ~ rank + is_mating + scale(age_bimon) + (1|id) + (1|year_bim), data = fem_rank, family = binomial, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))#NS; optimizer necessary to keep group
#Models with interactions
logit_fem2 = glmer(event ~ rank*scale(age_bimon) + is_mating  + (1|id) + (1|year_bim), data = fem_rank, family = binomial,
control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))#*; optimizer necessary to keep group
logit_fem3 = glmer(event ~ rank*is_mating + scale(age_bimon) + (1|id) + (1|group) + (1|year_bim), data = fem_rank, family = binomial,
                   control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))#NS; optimizer necessary to keep group


#Severe injuries (all models had singular fit when including group as random effect, so excluded)
#Simplest model 
logit_sev_F1 = glmer(is_severe ~ rank + is_mating + scale(age_bimon)  + (1|id) +  (1|year_bim), data = fem_rank, family = binomial)#ns
#Models with interaction
...
#-#-#-#-MALES#-##--
#Simplest model
logit_male1 = glmer(event ~ scale(tenure) + is_mating + scale(age_bimon) + (1|id) + (1|group) + (1|year_bim), data = male_rank, family = binomial)#**
#Models with interactions
logit_male2 = glmer(event ~ scale(tenure)*is_mating + scale(age_bimon)  + (1|id) + (1|group) + (1|year_bim), data = male_rank, family = binomial)#ns
logit_male3 = glmer(event ~ scale(tenure)*scale(age_bimon)  + is_mating + (1|id) + (1|group) + (1|year_bim), data = male_rank, family = binomial)#**
...
#Severe injuries
#Simplest model
logit_sev_M1 = glmer(is_severe ~ scale(tenure) + scale(age_bimon) + is_mating + (1|id) + (1|group)+ (1|year_bim), data = male_rank, family = binomial)#ns
#Model with interactions
logit_sev_M2 = glmer(is_severe ~ scale(tenure)*is_mating + scale(age_bimon) + (1|id) + (1|group) + (1|year_bim), data = male_rank, family = binomial)#ns
logit_sev_M3 = glmer(is_severe ~ scale(tenure)*scale(age_bimon)  + is_mating + (1|id) + (1|group) + (1|year_bim), data = male_rank, family = binomial)#*
...
#ALL INJURIES##
#Simplest model
logit_fem1 = glmer(event ~ scale(n_kin2) + is_mating + scale(age_bimon) + (1|id) + (1|group) + (1|year_bim), data = fem_data, family = binomial,
                   control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))#p = 0.059; convergence issues so optimizer used
#Models with interactions
logit_fem2 = glmer(event ~ scale(n_kin2)*scale(age_bimon)  + is_mating + (1|id) + (1|group) + (1|year_bim), data = fem_data, family = binomial,
                   control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))#ns; convergence issues so optimizer used
logit_fem3 = glmer(event ~ scale(n_kin2)*is_mating + scale(age_bimon) + (1|id) + (1|group) + (1|year_bim), data = fem_data, family = binomial,
                   control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))#ns; convergence issues so optimizer used


#Severe injuries (Models with singular fit so group excluded)
#Simplest model
logit_sev_F1 = glmer(is_severe ~ scale(n_kin2) + is_mating + scale(age_bimon)  + (1|id) + (1|year_bim), data = fem_data, family = binomial)#ns
#Models with interactions
...
#ALL INJURIES##
#Simplest model
logit_fem1.1 = glmer(event ~ scale(n_kin1) + is_mating + scale(age_bimon) + (1|id) + (1|group) + (1|year_bim), data = fem_data, family = binomial,
control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))#* ; convergence issues so optimizer used
#Models with interactions
logit_fem1.2 = glmer(event ~ scale(n_kin1)*scale(age_bimon) + is_mating  + (1|id) + (1|group) + (1|year_bim), data = fem_data, family = binomial, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))#ns; convergence issues so optimizer used
logit_fem1.3 = glmer(event ~ scale(n_kin1)*is_mating  + scale(age_bimon)  + (1|id) + (1|group) + (1|year_bim), data = fem_data, family = binomial, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))#ns; convergence issues so optimizer used


#Severe injuries (problems of singularity, so group excluded)
#Simplest model
logit_sev_F1.1 = glmer(is_severe ~ scale(n_kin1) + is_mating + scale(age_bimon) + (1|id) +  (1|year_bim), data = fem_data, family = binomial)#ns
#Model with interactions
logit_sev_F1.2 = glmer(is_severe ~ scale(n_kin1)*is_mating  + scale(age_bimon) + (1|id)  + (1|year_bim), data = fem_data, family = binomial)#p = 0.05
logit_sev_F1.3 = glmer(is_severe ~ scale(n_kin1)*scale(age_bimon) + is_mating + (1|id) + (1|year_bim), data = fem_data, family = binomial)#ns
...
rank_testF = lmerTest::lmer(scale(perc.rank) ~ rank + (1|id) + (1|group), data = female_rank_cor)#rankLow = -1.12, p < 0.01
...
rank_testM = lmerTest::lmer(scale(perc.rank) ~ scale(tenure) + (1|id) + (1|group), data = male_rank_cor)#tenure = 0.67, p < 0.01
...
fit = glmer(is_severe ~ scale(n_kin1)*is_mating + scale(age_bimon) + (1|id) + (1|year_bim), data = fem_data, family = binomial)
